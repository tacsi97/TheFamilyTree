...
namespace FamilyTree.Core.Extensions
{
    ...
    {
        public static async Task<IEnumerable<Business.Relationship>> GetRelationshipsIncludedIn(this IAsyncGraphRepository<Business.Relationship> asyncGraph, int familyTreeID)
        {
        ...
        public static async Task CreateParentRelationship(this IAsyncGraphRepository<Business.Relationship> asyncGraphRepository, Business.Person fromPerson, Business.Person toPerson, DateTime from, DateTime to)
        {
            using (var gc = new GraphClient(new Uri(DatabaseInfo.Uri), DatabaseInfo.UserName, DatabaseInfo.Password))
            {
